...
export async function startRepl(box: Box, options?: BoxREPLClientOptions): Promise<void> {
    ...
    onModelConfiguration: async () => {
      ...
      for (const group of groups) {
        for (const opt of group.options) {
          items.push({ label: opt.label, value: opt.value, description: dim(opt.value) });
        }
      }
      items.push({
        label: "Custom model",
      ...
      (rl as unknown as { _ttyWrite: Function })._ttyWrite = origTtyWrite;
      rl.resume();
      ...
      if (model === "__custom__") {
        try {
          model = await rl.question(dim("  Enter model identifier: "));
          model = model.trim();
        } catch {
          model = "";
        }
  ...
  if (stdin.isTTY) {
    ...
    stdin.on("keypress", (_str: string, key: { name?: string }) => {
      ...
      setImmediate(() => {
        if ((rl as unknown as { paused: boolean }).paused) return;
        const line = (rl as unknown as { line: string }).line ?? "";
        clearPreview();
  ...
  function processEvent(event: BoxREPLEvent): boolean {
    switch (event.type) {
      ...
      case "clear":
        ...
        break;
      case "open-url": {
        // Pass the URL as an argv entry (never through a shell) so metacharacters
        // in the box id / URL can't be interpreted by the host shell.
        if (process.platform === "win32") {
          execFile("cmd", ["/c", "start", "", event.url]);
        } else {
          execFile(process.platform === "darwin" ? "open" : "xdg-open", [event.url]);
        }
        break;
      }
      case "exit":
